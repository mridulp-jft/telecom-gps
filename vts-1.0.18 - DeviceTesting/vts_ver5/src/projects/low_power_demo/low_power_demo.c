...
/****************************************************************************************************/
/****************************************************************************************************/
/* This project was designed to work on a FRDM-KL26Z board, configured with PEE at 48Mhz            */
/*      Consult your Freedom board schematics for which jumper to use for current                   */
...
int main (void)
{
	...
	start();
        ...
        /*PTC3(SW1) is configured to wake up MCU from VLLSx and LLS modes, Interrup is ne*/
        LLWU_Init();
	...
	while(1)
	{
            ...
            while (test_num > 22 | test_num < 0){
               ...
               if ((SMC_PMSTAT & SMC_PMSTAT_PMSTAT_MASK)== 4){
                ...
                printf("E for CASE 14: Running benchmark 2 times in RUN  with CPO then without CPO\n\r");
                printf("F for CASE 15: Running benchmark 2 times in VLPR with CPO then without CPO\n\r");
            ...
            LED3_OFF;
#else
            printf("\n\n\r For FREEDOM board just press SW1 to exit\n\r\n");
#endif
            ...
            switch(test_num){
             ...
             case 1://VLLS0
                ...
                uart0_getchar(UART0_BASE_PTR);
                printf("\n\rPress SW1 to wake up from VLLS0\n\r ");
                clockMonitor(OFF);
                set_4_lp(0);
                enter_vlls0(0);
                break;
             ...
             case 9:// VLPW
              ...
              case 0xb://STOP
                ...
                uart0_getchar(UART0_BASE_PTR);
                  printf("Press SW1 to wake up from Normal Stop\n\r "); 
                clockMonitor(OFF);
                /*Enter stop mode*/
              ...
              case 0xc://PSTOP1 
                ...
                uart0_getchar(UART0_BASE_PTR);
                  printf("Press SW1 to wake up from Stop PSTOP1 \n\r "); 
                clockMonitor(OFF); 
                /*Enter stop mode
                * Partial Stop Option:  
            ...
            case 0xd://STOP PSTOP2
                ...
                uart0_getchar(UART0_BASE_PTR);
                  printf("Press SW1 to wake up from Stop PSTOP2\n\r "); 
                clockMonitor(OFF);
                /*Enter stop mode
                * Partial Stop Option:  
                *  0x00 = STOP - Normal Stop Mode
              ...
              case 0xe://COMPUTE MODE RUN
                ...
                for (i=0;i<CPO_LOOP_CNT;i++)
                    {
                     printf("Start benchmark in Compute Only (CPO) Mode --> \n\r");
                     uart0_putchar(UART0_BASE_PTR, ' ');       // ensure UART Tx buffer is empty
                     ...
                     if(MCM_CPO & MCM_CPO_CPOACK_MASK){
                       ...
                       printf("                   - Did you insert the benchmarking code? \n\r");
                       break;
                     }
                     /****/
                     printf(" Now running benchmark without CPO mode enabled. \n\r"); 
                     main_app(); //CoreMarks
                     printf("benchmark iterations = 0x%02X of 2 \n\r",i) ;
                  }                 
                  break;
              ...
              case 0xf://Coremark in VLPR
                ...
                for (i=0;i<2;i++)
                  {
                     printf("Starting running benchmark in VLPR with--- CPO mode enabled. \n\r   ");
                     ...
                     printf("Starting running benchmark in VLPR without CPO mode enabled. \n\r"); 
                     main_app(); //CoreMarks
                     printf("benchmark iterations = 0x%02X of 2 \n\r",i) ;
                  }
...
void SW_LED_Init(void)
{
    ...
    SIM_SCGC5 |= SIM_SCGC5_PORTE_MASK | SIM_SCGC5_PORTD_MASK ; /* PORT clock enablement */ 

   LED0_EN; // (PORTE_PCR29 = PORT_PCR_MUX(1))
   GPIOE_PDDR |= (1<<29);
   LED1_EN; // (PORTE_PCR31 = PORT_PCR_MUX(1))
   GPIOE_PDDR |= (1U<<31);
   LED2_EN; // (PORTD_PCR5 = PORT_PCR_MUX(1))
   GPIOD_PDDR |= (1<<5);
}
...
void Port_Init(int io_nmi){
  ...
  if (buttons_read == 0x8) {  // SW1 buttons not -pressed
    //disable debug
...
void portc_isr(void)
{
   if(MCM_CPO & MCM_CPO_CPOACK_MASK){
      MCM_CPO &= ~MCM_CPO_CPOREQ_MASK;
      while (MCM_CPO & MCM_CPO_CPOACK_MASK);                
      printf("         \n\r[portc_isr] Interrupt pulled OUT of Compute Mode  \n\r");               
  }
  ...
  printf("\n\r[portc_isr] portc3 ISF cleared\n\r");
  return;
}
...
__weak void main_app(void)
{
    ...
    printf("\n\n\rBenchmarking code has not been inserted.  Skipping function!! \n\n\r");                
}
