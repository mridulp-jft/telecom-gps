...
int32_t main(void)
{
    ...
    while(1) {
        ...
        if(strstr(g_u8RecData,"\"fw.bin\",0"))goto _APROM;
        ...
        *fsize++;*fsize++;*fsize++;*fsize++;*fsize++;*fsize++;*fsize++;
        ...
        memset(temp, 0, 100);
        if(filesize_int % firmwarechunksize == 0){
          times = (filesize_int/firmwarechunksize);
        }else{
        ...
        for(i = 0; i< times; i++){
          ...
          send_at(temp, "NO ACKNOWLEDGE", "NO ACKNOWLEDGE", "NO ACKNOWLEDGE\r\n");
          memset(temp, 0, 100);
          parse_g(g_u8RecData,1,2,'\n','\n',temp);
          strcat(temp, "\n");
