...
uint8_t g_u8RecData[g_u8RecDatamax] = 0;
uint32_t g_u8RecDataptr  = 0;
extern int fptr;
/* please check "targetdev.h" for chip specifc define option */
extern uint32_t g_au32TMRINTCount;
extern uint8_t firmwaredata[120];
extern int f;
/*---------------------------------------------------------------------------------------------------------*/
    ...
    while(!UART_GET_RX_EMPTY(UART1)) 
    {
      ...
      //if(uart1data != 0){
        if(f==0)
          ...
          firmwaredata[fptr] = uart1data;fptr++;}
        g_u8RecDataptr++;
//        if(g_u8RecDataptr == g_u8RecDatamax-1){
//          memset(g_u8RecData,0,g_u8RecDatamax);
//          g_u8RecDataptr = 0;
//        }
      }
    //}
...
void send_at(char* cmd, char* response1, char* response2, char* response3)
{   g_u8RecDataptr = 0;
    g_au32TMRINTCount = 0;
    while ((UART_N->FSR & UART_FSR_TX_FULL_Msk));
    UART_N->THR = 0x0A;
    ...
    while(!((strstr(g_u8RecData, response1)) ||  (strstr(g_u8RecData, response2)) ||  (strstr(g_u8RecData, response3))) && ((g_au32TMRINTCount < 2)) );    
}
