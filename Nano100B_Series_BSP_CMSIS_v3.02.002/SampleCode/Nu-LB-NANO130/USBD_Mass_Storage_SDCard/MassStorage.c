...
void MSC_Init(void)
{
    ...
    // Initial SDCARD
    SDCARD_Open();
    // Get SDCard total sector
    g_TotalSectors = GetLogicSector();
    //printf("SD_DiskInfo0.totalSectorN = 0x%x\n", g_TotalSectors);
}
...
void MSC_Write(void)
{
    ...
    if (g_u32Length > EP3_MAX_PKT_SIZE) {
        ...
        if (g_u32Address >= (STORAGE_DATA_BUF + STORAGE_BUFFER_SIZE)) {
            SpiWrite(g_u32DataFlashStartAddr, STORAGE_BUFFER_SIZE, (uint8_t *)STORAGE_DATA_BUF);
        ...
        if ((g_sCBW.u8OPCode == UFI_WRITE_10) || (g_sCBW.u8OPCode == UFI_WRITE_12)) {
            ...
            if (len)
                SpiWrite(g_u32DataFlashStartAddr, len, (uint8_t *)STORAGE_DATA_BUF);
        }
        ...
        if (g_u8BulkState == BULK_CBW) {
            ...
            case UFI_READ_10: {
                ...
                if(g_u32Length == 0) {
                    ...
                    Dcount = (get_be32(&g_sCBW.au8Data[4])>>8) * UDC_SECTOR_SIZE;
