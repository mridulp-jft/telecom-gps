...
void UART_ClearIntFlag(UART_T* uart , uint32_t u32InterruptFlag)
{
    ...
    if(u32InterruptFlag & (0x1ul << (2))) {  
        uart->FSR |= (0x1ul << (6)) | (0x1ul << (5)) | (0x1ul << (5));
        uart->TRSR |= (0x1ul << (0));
    }
...
void UART_Open(UART_T* uart, uint32_t u32baudrate)
{
    ...
    uint32_t u32ClkTbl[4] = {(12000000UL), (32768UL), 0, (12000000UL)};
    uint32_t u32Baud_Div;
    uint32_t u32SrcFreq;
    uint32_t u32SrcFreqDiv;

    u8UartClkSrcSel = (((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKSEL1 & (0x3ul << (0))) >> (0);

    u32SrcFreq = u32ClkTbl[u8UartClkSrcSel];

    u32SrcFreqDiv = (((((CLK_T *) (((uint32_t)0x50000000) + 0x00200))->CLKDIV0 & (0xful << (8))) >> (8)) + 1);
...
void UART_SetTimeoutCnt(UART_T* uart, uint32_t u32TOC)
{
    uart->TMCTL = (uart->TMCTL & ~(0x1fful << (0)))| (u32TOC);
    uart->IER |= (0x1ul << (4));
}
...
void UART_SelectIrDAMode(UART_T* uart, uint32_t u32Buadrate, uint32_t u32Direction)
{
    ...
    uart->IRCR    &=  ~(0x1ul << (5));
    uart->IRCR |=     (0x1ul << (6));
    uart->IRCR    = u32Direction ? uart->IRCR | (0x1ul << (1)) : uart->IRCR &~ (0x1ul << (1));
    uart->FUN_SEL = (0x2 << (0));
}
...
void UART_SelectRS485Mode(UART_T* uart, uint32_t u32Mode, uint32_t u32Addr)
{
    uart->FUN_SEL = (0x3 << (0));
    uart->ALT_CTL = 0;
    uart->ALT_CTL |= u32Mode | (u32Addr << (24));
}
...
void UART_SelectLINMode(UART_T* uart, uint32_t u32Mode, uint32_t u32BreakLength)
{
    ...
    uart->FUN_SEL = (0x1 << (0));
    uart->ALT_CTL &= ~((0x7ul << (0)) | (0x1ul << (6)) | (0x1ul << (7)));
    uart->ALT_CTL |= u32BreakLength & (0x7ul << (0));
