...
{									uart_putchar(UART1_BASE_PTR, 0x1A);clear();delay(10);
									...
									delay(100);
									clear();//memset(uart_string_gsm,0,5000);868004022692985
...
//	else{breach=0;}
//	if(voltagecbc<4000  ||  (GPIOB_PDIR & 0x00000002)){
...
sprintf(num2,"%d,%ld",voltagecbc,rtcticks2);
//send_modem(num2);	
...
if	((sleep_flag != 1  || (aliveping > 5) ) )			
{
	...
	if((aliveping>5))	
	{
		...
		aliveping=0;
		memset(data,0,strlen(data));	
		strcat(data,imei);strcat(data,",ALIVE,");strcat(data,num2);strcat(data,",F=");strcat(data,fuelstr);strcat(data,",I=");strcat(data,ignstr);strcat(data,",\n\n\n"); 
	}	
...
__inline	void send_message1(char * data2)
	{
									memset(at_,0,100);
									sprintf(at_, "\nAT+CMGS=\"%s\"\r\n" ,"8287324005");
									send_MSG(at_, ">", "ERROR\r\n" , "ERROR:",tmeout);	
