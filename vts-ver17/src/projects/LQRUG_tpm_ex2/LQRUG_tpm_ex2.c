 ...
 *              - just CH2 (PTA5 a.k.a. D5) outputs used,
 ...
 *              - DMA destination address is TPM0_C2V
 *              - DMA source is incremented after each request, destination not
 *              - TPM0 and DMA running after VLPS entering,
 *              - Waveforms can be cycled through manualy by pressing SW1 on FRDM-KL26Z
 */
...
int main (void)
{
  ...
  while (1)
  {
    ...
    for (i = 0; i < 64; i++)
    {
      temp16 = p_au16PwmDuties[i]>>1;
      au16PwmDuties[i] = temp16;
    }
    DMAMUX0_CHCFG0 |= DMAMUX_CHCFG_ENBL_MASK;
...
void PORT_Init(void)
{
  enable_irq(31); set_irq_priority(31, 1);
  PORTA_PCR5 = PORT_PCR_ISF_MASK |PORT_PCR_MUX(0x3)| PORT_PCR_DSE_MASK;
  PORTC_PCR3 = PORT_PCR_ISF_MASK |PORT_PCR_MUX(0x1)| PORT_PCR_PS_MASK| PORT_PCR_PE_MASK| PORT_PCR_PFE_MASK| PORT_PCR_IRQC(10);  //SW1 on FRDM-KL26Z, set for falling edge interrupts.
