...
extern char uart_string_obd[3000],num2[20],uart_string_gsm[5000],data[1500];
char *test;
extern int k2,l2,k23;
//long time=999999;
/********************************************************************/
/*
...
{//int32 counter=19999;
		...
		//	do
			...
			{
				uart_string_obd[k23]=UART_D_REG(UART0_BASE_PTR);
				k23++;
			//	counter=19999;
		//	if(k2>4990)k2=0;
			}
//				else{counter--;
//						}
			}//`while((UART_S1_REG(UART1_BASE_PTR) & UART_S1_RDRF_MASK)  || (counter>1));
...
{//int32 counter=19999;
		...
		//	do
			...
			{uart_string_gsm[k2]=UART_D_REG(UART1_BASE_PTR);k2++;
			//	counter=19999;
		//	if(k2>4990)k2=0;
			}
//				else{counter--;
...
//			{uart_string_gps[k]=UART_D_REG(UART2_BASE_PTR);k++;l=9999;if(k>298){k=0;}}
//				else{
//							l--;if(l==10){}
//						}
...
//				        	||(strstr(uart_string_gps, "ERROR:"))
//							));
//printf("uart_string_gps -> %s",uart_string_gps);
}
...
char uart_getchar (UART_MemMapPtr channel)
{
      ...
      while (!((UART_S1_REG(channel) & UART_S1_RDRF_MASK)    ));//{timeouttt--;}
 //   ||   (timeouttt < 1 ) 
...
{int to=19999;
      ...
      while(!((UART_S1_REG(channel) & UART_S1_TDRE_MASK)    ||  ( to <= 1 ))){to--;}
...
void uart0_init (UART0_MemMapPtr uartch, int sysclk, int baud)
{
    ...
    if (baud_diff < ((baud_rate / 100) * 3))
    {
        ...
        UART0_BDL = (uint8)(sbr_val & UART0_BDL_SBR_MASK);
			...
			UART0_C2_REG(uartch) |= UART0_C2_RIE_MASK;			//newly added
...
{int t0=19999;
      /* Wait until space is available in the FIFO */
      while(!(UART0_S1_REG(channel) & UART0_S1_TDRE_MASK)||  ( t0 <= 1 )){t0--;}
