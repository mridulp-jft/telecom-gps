...
__inline void RELAY_init(void)
{
    ...
    SIM_SCGC5 |= SIM_SCGC5_PORTC_MASK ;      //Turn on clock to PORTE & D module
    PORTC_PCR10 = PORT_PCR_MUX(0x1);            //Set the PTA1 pin multiplexer to GPIO mode
  //  PORTA_PCR2 = PORT_PCR_MUX(0x1);            //Set the PTA1 pin multiplexer to GPIO mode
	
    GPIOC_PSOR |= RESETSTN_SHIFT ;      //Set the initial output state to high	
    GPIOC_PDDR |= RESETSTN_SHIFT ;  //Set the pin's direction to output	
