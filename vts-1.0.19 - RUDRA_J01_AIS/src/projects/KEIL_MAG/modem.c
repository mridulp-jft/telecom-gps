...
__inline void save_licenceplate(){
...
send_GSM("\nAT+FSCREATE=C:\\LICPLATE.TXT\r\n", "OK", "ERROR\r\n" , "ERROR:",tmeout);
	...
	send_modem("|LICPLATE|");
		for(k=0;k<strlen(licenceplate);k++)
		{
			uart_putchar(UART1_BASE_PTR, licenceplate[k]);// uart0_putchar(UART0_BASE_PTR, interval[k]);//clear();
		}	
			send_modem("|");			
...
memset(at_,0,100);
		...
		send_GSM(at_, "OK", "ERROR\r\n" , "ERROR:",10);	
if(strstr(uart_string_gsm,"ION")){IGNITION_ON;FUEL_ON;ign = 1;}
else if(strstr(uart_string_gsm,"IOFF")){IGNITION_OFF;FUEL_OFF;ign = 0;}
...
__inline void read_licenceplate()
{
		memset(at_,0,100);
		sprintf(at_, "\nAT+FSREAD=C:\\LICPLATE.TXT,0,100,1\r\n");		
		send_GSM(at_, "OK", "ERROR\r\n" , "ERROR:",10);	
		parse_g(uart_string_gsm, 2,3, '|', '|',licenceplate);
