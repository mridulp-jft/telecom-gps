...
extern char uart_string_gps[500],num2[20],uart_string_gsm[5000],data[1500];
extern char id[50];
char *test;
extern int k2,l2;
int uart0buffinc=0;
...
intrtmr=0;
		...
		if((UART_S1_REG(UART0_BASE_PTR) & UART0_S1_RDRF_MASK))
			{
				id[uart0buffinc]=UART_D_REG(UART0_BASE_PTR);
				uart0buffinc++;
				intrtmr = 0;
			}
 ...
 *  the received character
 */
int8 uart0_getchar (UART0_MemMapPtr channel)
{
      /* Wait until character has been received */
      while (!(UART0_S1_REG(channel) & UART0_S1_RDRF_MASK));
