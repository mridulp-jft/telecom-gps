						...
						cond=mystrstr(uart_string_gsm,'>');
								...
								{	
									...
									do
																...
																}while(!(stcond2 || stcond3 || stcond4 || stcond1 || (tout > 7) ) ); //|| (timeout < 1)   
																delay(10);clear();
														uart_putchar(UART1_BASE_PTR,0x1A);
												delay(10);
										uart_putchar(UART1_BASE_PTR, 0x1A);uart_putchar(UART1_BASE_PTR, 0x1A);uart_putchar(UART1_BASE_PTR, 0x1A);
...
if(rtcticks2>=1500){
	...
	delay(100);
					strcat(data, "\n");
					strcat(data, imei);
					strcat(data, ",Hey, This is your device restrting\n");
...
sprintf(num2,"<%d><%ld>",aliveping,rtcticks2);
//send_modem(num2);	
							...
							send_GSM("\r\nAT+CIPSEND\r\n", ">", "ERROR" , "CONNECT",tmeout);	
								cond =mystrstr(uart_string_gsm,'>');
									...
									do
																...
																}while(!(q1 || q2 || q3 || q4 || (tout > 7) ) ); //|| (timeout < 1)   
																delay(10);clear();
														uart_putchar(UART1_BASE_PTR,0x1A);
												delay(10);
										uart_putchar(UART1_BASE_PTR, 0x1A);uart_putchar(UART1_BASE_PTR, 0x1A);uart_putchar(UART1_BASE_PTR, 0x1A);
