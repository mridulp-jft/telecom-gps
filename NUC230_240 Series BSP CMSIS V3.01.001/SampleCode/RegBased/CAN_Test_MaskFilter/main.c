...
int main(void)
{
    ...
    MaskMsg.u8Xtd    = 0;               /* 1: 29-bit Extended Identifier or 0:11-bit Standard Identifier */
    ...
    MaskMsg.u32Id    = 0;       /* Set the Message Identifier  */

    /* Configures Mask as the message object */
//    CAN_SetMsgObjMask(tCAN1, MSG(0), &MaskMsg);

////    /* Set Rx message object */
//    CAN_SetRxMsgObj(tCAN1, MSG(0), CAN_STD_ID, 0x7FF , TRUE);
...
//    NVIC_EnableIRQ(CAN1_IRQn);
    ...
    while(1) {
        ...
        tMsg.Id       = 0x348;  /* Set the Message Identifier  */
        tMsg.DLC      = 8;                          /* Set the Data Length Code  */
				tMsg.Data[0]  = 0x14;
				tMsg.Data[1]  = 0x03;
				tMsg.Data[2]  = 0x06;
				tMsg.Data[3]  = 0x0;
...
//CAN_STOP();
        ...
        if(CAN_SetTxMsgObj(tCAN1, MSG(1), &tMsg) < 0) {
            printf("Set Tx Msg Object failed\n");
            return 0;
        }

//        /* Set transmit request bit */
        CAN_SetTxRqst(tCAN1, MSG(1));

        /* Check the message has been successfully transmitted */
        while((tCAN1->STATUS & CAN_STATUS_TXOK_Msk) == 0);
        /* Clear TXOK flag */
        tCAN1->STATUS &= ~CAN_STATUS_TXOK_Msk;
...
void SYS_Init(void)
{
    ...
    /* Enable module clock */
    CLK->APBCLK = CLK_APBCLK_UART0_EN_Msk
                  | CLK_APBCLK_CAN0_EN_Msk| CLK_APBCLK_CAN1_EN_Msk;
