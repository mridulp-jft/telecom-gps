...
int sleep_flag=0;
int32 tmeout=6,int_erval=1; //10 SECS TIMEOUT
...
//}
		...
		aliveping=0;
		memset(data,0,strlen(data));	
		strcat(data,imei);strcat(data,",ALIVE,");strcat(data,num2);strcat(data,",SS=");strcat(data,fuelstr);		strcat(data,",AC=");	
		strcat(data,acstr);
		strcat(data,",I=");	
...
void Triggerus()
{
	...
	memset(sonictemp, 0, 20);
	memset(sonicvalue, 0, 20);
	memset(sonicsensor,0,10);
//for(;;){
//			UART_S1_REG(UART0_BASE_PTR) |= (UART_S1_OR_MASK | UART_C2_ILIE_MASK );
//			if((UART_S1_REG(UART0_BASE_PTR) & UART0_S1_RDRF_MASK))
//			{
//				sonictemp[sonictempptr]=UART_D_REG(UART0_BASE_PTR);
//				sonictempptr++;
//			}
//			if((int32)sonictemp[sonictempptr] == 255)
//				{break;}
////				else if((int32)sonictemp[sonictempptr] == 0)
////				{break;}
//}
...
parse_g(sonictemp, 2, 3, (char)0xFF, (char)0xFF, sonicvalue);
decimal = 0;
sprintf(sonicsensor,"%x%x",sonicvalue[0],sonicvalue[1]);
sonicvaluedecimal = convtodecnum(sonicsensor);
memset(sonicsensor,0,10);
sprintf(sonicsensor,"%d",sonicvaluedecimal);
//send_modem(sonicsensor);
