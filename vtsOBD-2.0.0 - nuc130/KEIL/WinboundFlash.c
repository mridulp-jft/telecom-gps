...
{uint32_t u32HCLKFreq;
    ...
    SYS->GPC_MFP &= ~(SYS_GPC_MFP_PC9_Msk | SYS_GPC_MFP_PC10_Msk | SYS_GPC_MFP_PC11_Msk | SYS_GPC_MFP_PC12_Msk);
    SYS->GPC_MFP |= SYS_GPC_MFP_PC8_SPI1_SS0 | SYS_GPC_MFP_PC10_SPI1_MISO0 | SYS_GPC_MFP_PC11_SPI1_MOSI0 | SYS_GPC_MFP_PC9_SPI1_CLK;
  
    /* Init GPIO for SPI Flash Port, set PE1, PE2, PE3 and PE4 for SPI0 */
//    SYS->GPC_MFP |= (SYS_PC_L_MFP_PC8_MFP_SPI1_SS0 | SYS_PE_L_MFP_PE2_MFP_SPI0_SCLK |
//                      SYS_PE_L_MFP_PE3_MFP_SPI0_MISO0 | SYS_PE_L_MFP_PE4_MFP_SPI0_MOSI0);
    ...
    SPI_FLASH_PORT->CNTRL = (SPI_FLASH_PORT->CNTRL & ~(SPI_CNTRL_SLAVE_Msk | SPI_CNTRL_LSB_Msk | SPI_CNTRL_CLKP_Msk | SPI_CNTRL_TX_BIT_LEN_Msk | SPI_CNTRL_TX_NEG_Msk | SPI_CNTRL_RX_NEG_Msk)) | SPI_CNTRL_TX_NEG_Msk;
    ...
    SPI_FLASH_PORT->DIVIDER = (SPI_FLASH_PORT->DIVIDER & ~SPI_DIVIDER_DIVIDER_Msk) | (0x14 << SPI_DIVIDER_DIVIDER_Pos);
...
unsigned int SpiFlash_ReadMidDid(void)
{
   ...
   // SPI_FLASH_PORT->SSR = (SPI_FLASH_PORT->SSR & ~SPI_SSR_SSR_Msk);
    ...
    au32SourceData = 0x90;
    SPI_FLASH_PORT->TX[0] = au32SourceData;
    SPI_FLASH_PORT->CNTRL |= SPI_CNTRL_GO_BUSY_Msk;

    // wait
    while (SPI_FLASH_PORT->CNTRL & SPI_CNTRL_GO_BUSY_Msk) {}

    // configure transaction length as 24 bits
    SPI_FLASH_PORT->CNTRL = (SPI_FLASH_PORT->CNTRL & ~SPI_CNTRL_TX_BIT_LEN_Msk) | (0x18 << SPI_CNTRL_TX_BIT_LEN_Pos);
    ...
    SPI_FLASH_PORT->CNTRL = (SPI_FLASH_PORT->CNTRL & ~SPI_CNTRL_TX_BIT_LEN_Msk) | (0x10 << SPI_CNTRL_TX_BIT_LEN_Pos);
    ...
    // dump Rx register
    au32DestinationData = SPI_FLASH_PORT->RX[0];
...
//unsigned int SpiFlash_Writetoaddress(void)
//{
//    unsigned int au32SourceData;
//    unsigned int au32DestinationData;
